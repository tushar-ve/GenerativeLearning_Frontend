...
const Login = () => {
  ...
  return (
    ...
    <label className='sub'>E-mail</label><br/>
      ...
      <input placeholder='Pa**word'  type="password" name='password' />
      <p style={{fontSize:'10px', textAlign:'left', color:'#B02525', marginLeft:'12px'}}>forgot Password?</p>
    </div>
    <button type='submit' className='login-button'>Login</button>
    </form>
    <p style={{fontSize:'10px', textAlign:'left',  marginLeft:'12px'}}> Don't have an account? <span style={{color:'#B02525', cursor:'pointer'}} onClick={()=>navigate('/register')}>SignUp</span></p>
    {error.status?<alert style={{fontSize:'15px', textAlign:'left',color:'#B02525',  marginLeft:'12px'}}>{error.msg}</alert>:'' }
    </div>    
    <div>    
    </div>
    </div>
